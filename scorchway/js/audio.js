...
const AU = {
  ...
  engine(s, nitro, slip){
    ...
    const t = this.ctx.currentTime, menu = G.state === 'menuplay';
    const gear = Math.min(4, Math.floor(s * 5)), within = s * 5 - gear, rpm = .22 + Math.min(1, within) * .78 + (nitro ? .15 : 0);
    const f = 34 + rpm * 70 + gear * 5;
    L.o1.frequency.setTargetAtTime(f, t, .04); L.o2.frequency.setTargetAtTime(f * .5, t, .04);
    L.ef.frequency.setTargetAtTime(300 + rpm * 1400 + (nitro ? 900 : 0), t, .05);
    L.eg.gain.setTargetAtTime(menu ? .015 : .07 + (nitro ? .03 : 0), t, .08);
    L.sg.gain.setTargetAtTime(menu ? 0 : clamp((slip - .12) * .35, 0, .09), t, .05);
    L.sf.frequency.setTargetAtTime(1500 + slip * 1600, t, .05);
    // siren / rotor / fire by proximity
    ...
    const sv = menu ? 0 : clamp(1 - cop / 900, 0, 1) * .035;
    L.sig.gain.setTargetAtTime(sv, t, .1);
    L.so.frequency.setTargetAtTime(((G.rt * 1.6) | 0) % 2 ? 960 : 720, t, .05);
    const hv = menu ? 0 : clamp(1 - heli / 1000, 0, 1) * .12;
    L.rg.gain.setTargetAtTime(hv * .5, t, .1); L.rlg.gain.setTargetAtTime(hv * .5, t, .1);
    let fire = 0; for(let i = 0; i < G.F.length; i += 4){ const f2 = G.F[i]; if(Math.abs(f2.x - G.camX) < 700 && Math.abs(f2.y - G.camY) < 500) fire++; }
    L.fg.gain.setTargetAtTime(menu ? 0 : clamp(fire / 60, 0, 1) * .05 * (.6 + .4 * Math.random()), t, .03);
  },
